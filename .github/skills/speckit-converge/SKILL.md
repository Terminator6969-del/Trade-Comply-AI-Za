...
description: Assess the current codebase against the feature's spec, plan, and tasks, then append any remaining unbuilt work as new tasks to tasks.md so implement can complete it. Run after speckit-implement.
---
...
**Convergence Findings Table:**
| ID | Gap Type | Severity | Source Ref | Finding | Suggested Fix |
|---|---|---|---|---|---|
...
5. Never reuse or renumber existing IDs. If a prior Convergence phase exists, add a new, separately-numbered one below it — do not touch the old one.
...
Check for `hooks.after_converge` in `.specify/extensions.yml`
